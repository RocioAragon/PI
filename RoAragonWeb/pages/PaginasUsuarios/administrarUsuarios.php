    ...
    <div class="container">
        ...
        <div class="container text-center">
            ...
            if (mysqli_connect_errno()) {
                ...
                if ($admin) {
                    echo "<form action='interfazInsertarUsuario.php'>
                    <input type='submit' class=\"btn btn-dark\" value='Registrar un nuevo usuario' /></form><br>";
                    echo "</br>";
                    $consulta = "SELECT * FROM usuarios order by apellidos";

                    if ($result = mysqli_query($conn, $consulta)) {                   // Salida por pantalla
                        echo "<table class='m-auto w-100 table'> <thead class=\"thead-dark\"><tr> <th>DNI</th> <th>Nombre</th> <th>Apellidos</th> <th>Administrador</th><th>Activado</th><th></th><th></th><th></th></tr></thead>";
                        while ($row = mysqli_fetch_array($result)) {
                            if ($row['activado'] == 1) {
                                $activo = "<i class='fas fa-check-square' style='color:rgb(25, 216, 0);'></i>";
                            } else {
                                $activo = "<i class='far far fa-square'></i>";
                            }
                            if ($row['administrador'] == 1) {
                                $esAdmin = "<i class='fas fa-check-square' style='color:rgb(25, 216, 0);'></i>";
                            } else {
                                $esAdmin = "<i class='far far fa-square'></i>";
                            }
                            echo "<tr><td>" . $row['dni'] . "</td><td>" . $row['nombre'] . "</td><td>" .
                                $row['apellidos'] . "</td><td>" . $esAdmin . "</td><td>" . $activo . "</td><td>" .
                                "</td><td><form action='interfazEditarUsuario.php' method='post'>" .
                                "<input type='text' id='dniB' name='dniB' value=" . $row['dni'] . " hidden><input type='submit' class=\"btn btn-dark\" value='Editar'></form></td>".
                                "<td><form action='eliminarUsuario.php' method='post'><input type='text' id='dniB' name='dniAntiguo' value=" . $row['dni'] . " hidden><input type='submit' class=\"btn btn-dark\" value='Eliminar'></form></td></tr>";
                        }
                        echo "</table>";
                    }
                    mysqli_close($conn);
