    ...
    <div class="container">
        ...
        <div class="container text-center">
            ...
            if (mysqli_connect_errno()) {
                ...
                if ($admin) {					
                    $consulta = "SELECT pedidos.dni, count(pedidos.numero_pedido), usuarios.nombre FROM pedidos, usuarios WHERE pedidos.dni = usuarios.dni GROUP BY dni";

                    if ($result = mysqli_query($conn, $consulta)) { // Salida por pantalla
						echo "<table class='m-auto w-100 table'> <thead class=\"thead-dark\"> <tr> <th>DNI</th> <th>Nombre</th> <th>Cantidad Pedidos</th><th></th></tr></thead>";
						while($row = mysqli_fetch_array($result)){
                            echo "<tr>";
                            echo "<td>". $row['dni'] . "</td><td>" . $row['nombre'] . "</td><td>" . $row[1] . "</td>";
                            echo "<td><br><form action='verPedidos.php' method='post'><input name='dni' type='text' value='". $row['dni'] ."' hidden /><input class=\"btn btn-dark\" type='submit' value='Ver Pedidos'/></form></td>";
                            echo "<br>";
                            echo"</tr>";
                        }
						echo "</table>";
